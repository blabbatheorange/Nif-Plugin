...
bool NifImporter::AddNoteTracks(float time, std::wstring name, std::wstring target, NiTextKeyExtraDataRef textKeyData, bool loop)
{
   if (textKeyData == NULL)
      return false;
...
bool KFMImporter::ImportAnimation()
{
   ...
   for(vector<NiControllerSequenceRef>::iterator itr = kf.begin(); itr != kf.end(); ++itr){
      ...
      AddNoteTracks( time, wide(cntr->GetName()), wide(textName), textKeyData, (CYCLE_LOOP == cntr->GetCycleType()) );
      ...
      for (vector<ControllerLink>::iterator lnk=links.begin(); lnk != links.end(); ++lnk)
      {
         ...
         if (mergeNonAccum && wildmatch(_T("* NonAccum"), wide(name))) 
         {
         ...
         if (strmatch(wide(type), _T("NiTransformController")))
         {
            Control *c = ai.GetTMController(name);
            if (NULL == c)
               continue;

            INode *n = GetNode(wide(name));
         ...
         else if (strmatch(wide(type), _T("NiGeomMorpherController")))
         {
            ...
            if (!var2.empty())
            {
               if (INode *n = GetNode(wide(name)))
               {
                  if (Modifier* mod = GetMorpherModifier(n))
                  {
                     int idx = -1;
                     for (int i=1; i<=100; ++i) {
                        if (strmatch(wide(var2), MorpherGetName(mod, i))) {
                           idx = i;
                           break;
                        }
                     }
         ...
         else if (strmatch(wide(type), _T("NiKeyframeController")))
         {
            Control *c = ai.GetTMController(name);
            if (NULL == c)
               continue;
...
bool AnimationImport::ImportGeoMorph(INode *n, NiGeomMorpherControllerRef ctrl, float time)
{
	...
	{
		std::wstring frameName = (ni.nifVersion >= VER_10_1_0_106) ? wide(data->GetFrameName(i)) : FormatString(_T("Frame #%d"), i);
		...
		geoNode->SetName(FormatText(_T("Morph: %s"), name));
	...
	)
{
	...
	if ( ImpNode *node = ni.i->CreateNode() )
	{
		...
		tnode->SetUserPropBool(_T("np_ignore"), TRUE);
