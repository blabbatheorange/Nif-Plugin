...
struct NiNodeNameEquivalence : public NumericStringEquivalence
{
   bool operator()(const NiNodeRef& n1, const NiNodeRef& n2) const { 
      return NumericStringEquivalence::operator()(wide(n1->GetName()), wide(n2->GetName()));
   }
};
...
static void BuildControllerRefList(NiNodeRef node, map<std::wstring,int>& ctrlCount)
{
   ...
   for (list<NiTimeControllerRef>::iterator itr = ctrls.begin(), end = ctrls.end(); itr != end; ++itr) {
      ...
      for (list<NiNodeRef>::iterator nitr = nlist.begin(); nitr != nlist.end(); ++nitr){
         std::wstring name = wide((*nitr)->GetName());
         map<std::wstring,int>::iterator citr = ctrlCount.find(name);
...
void NifImporter::ImportBipeds(vector<NiNodeRef>& nodes)
{
   ...
   try 
   {
      vector<NiNodeRef> bipedRoots = SelectNodesByName(nodes, _T("Bip??"));
      ...
      {
         std::wstring bipname = wide((*bipedItr)->GetName());
         std::wstring match = bipname + _T("*");
         ...
         BOOL arms = (CountNodesByName(bipedNodes, FormatText(_T("%s L UpperArm"), bipname.c_str())) > 0) ? TRUE : FALSE;
         BOOL triPelvis = bipedTrianglePelvis ? TRUE : FALSE;
         int nnecklinks=CountNodesByName(bipedNodes, FormatText(_T("%s Neck*"), bipname.c_str()));
         int nspinelinks=CountNodesByName(bipedNodes, FormatText(_T("%s Spine*"), bipname.c_str()));
         int nleglinks = 3 + CountNodesByName(bipedNodes, FormatText(_T("%s L HorseLink"), bipname.c_str()));
         int ntaillinks = CountNodesByName(bipedNodes, FormatText(_T("%s Tail*"), bipname.c_str()));
         int npony1links = CountNodesByName(bipedNodes, FormatText(_T("%s Ponytail1*"), bipname.c_str()));
         int npony2links = CountNodesByName(bipedNodes, FormatText(_T("%s Ponytail2*"), bipname.c_str()));
         int numfingers = CountNodesByName(bipedNodes, FormatText(_T("%s L Finger?"), bipname.c_str()));
         int nfinglinks = CountNodesByName(bipedNodes, FormatText(_T("%s L Finger0*"), bipname.c_str()));
         int numtoes = CountNodesByName(bipedNodes, FormatText(_T("%s L Toe?"), bipname.c_str()));
         int ntoelinks = CountNodesByName(bipedNodes, FormatText(_T("%s L Toe0*"), bipname.c_str()));
         BOOL prop1exists = CountNodesByName(bipedNodes, FormatText(_T("%s Prop1"), bipname.c_str())) ? TRUE : FALSE;
         BOOL prop2exists = CountNodesByName(bipedNodes, FormatText(_T("%s Prop2"), bipname.c_str())) ? TRUE : FALSE;
         BOOL prop3exists = CountNodesByName(bipedNodes, FormatText(_T("%s Prop3"), bipname.c_str())) ? TRUE : FALSE;
         int forearmTwistLinks = CountNodesByName(bipedNodes, FormatText(_T("%s L Fore*Twist*"), bipname.c_str()));
         int upperarmTwistLinks = CountNodesByName(bipedNodes, FormatText(_T("%s L Up*Twist*"), bipname.c_str()));
         int thighTwistLinks = CountNodesByName(bipedNodes, FormatText(_T("%s L Thigh*Twist*"), bipname.c_str()));
         int calfTwistLinks = CountNodesByName(bipedNodes, FormatText(_T("%s L Calf*Twist*"), bipname.c_str()));
         int horseTwistLinks = CountNodesByName(bipedNodes, FormatText(_T("%s L Horse*Twist*"), bipname.c_str()));
         ...
         if (master)
         {
            ...
            RenameNode(gi, FormatText(_T("%s L ForeTwist"), bipname), FormatText(_T("%s L ForearmTwist"), bipname));
            RenameNode(gi, FormatText(_T("%s R ForeTwist"), bipname), FormatText(_T("%s R ForearmTwist"), bipname));
            RenameNode(gi, FormatText(_T("%s R LUpArmTwist"), bipname), FormatText(_T("%s L UpperArmTwist"), bipname));
            RenameNode(gi, FormatText(_T("%s R LUpArmTwist"), bipname), FormatText(_T("%s R UpperArmTwist"), bipname));

            if (NiNodeRef nifBip = FindNodeByName(nodes, narrow(bipname)))
            {
...
static bool HasBipedPosDOF(LPCTSTR name)
{
   ...
   return (  wildcmpi(_T("Bip?? ? Clavicle"), name) 
      || wildcmpi(_T("Bip?? ? Toe?"), name) 
      || wildcmpi(_T("Bip?? ? Finger?"), name) 
      || wildcmpi(_T("Bip?? *Twist*"), name) 
      );
}
...
static float CalcScale(INode *bone, NiNodeRef node, vector<NiNodeRef>& children)
{
   ...
   if (n > 0) 
   {
      ...
      for (int i = 0; i<n; i++)
      {
         ...
         if (NiNodeRef child2 = FindNodeByName(children, string(narrow(child->GetName())))){
...
void NifImporter::AlignBiped(IBipMaster* master, NiNodeRef node)
{
   ...
   TSTR s1 = FormatText(_T("Processing %s:"), name.c_str());
   TSTR s2 = FormatText(_T("Processing %s:"), name.c_str());
   ...
   if (bone != NULL) 
   {
      ...
      s1 += FormatText(_T(" ( %s)"), PrintMatrix3(m).data());
      ...
      else if (INode *pnode = bone->GetParentNode())
      {
         ...
         if (!strmatch(wide(parent->GetName()), pnode->GetName())) {
      ...
      s2 += FormatText(_T(" ( %s)"), PrintMatrix3(bone->GetNodeTM(0)).data());
   }
   else
   {
      ImportBones(node, false);
   }
   for (wchar_t *p = s1.dataForWrite(2048); *p != 0; ++p) if (isspace(*p)) *p = ' ';
   for (wchar_t *p = s2.dataForWrite(2048); *p != 0; ++p) if (isspace(*p)) *p = ' ';
   OutputDebugString(s1 + _T("\n"));
   OutputDebugString(s2 + _T("\n"));
...
{
	//	Create Skeleton as Dummy Helper Objects
	/*if (DummyObject *ob = (DummyObject *)gi->CreateInstance(HELPER_CLASS_ID, Class_ID(DUMMY_CLASS_ID, 0))) {
      ...
      if (INode *n = CreateImportNode(name.c_str(), ob, NULL)) {
		 ...
		 n->SetBoneNodeOnOff(true, 0);
         return n;
		}
	}*/

	//	Create Skeleton as Bone Objects
	if (Object *ob = (Object *)gi->CreateInstance(GEOMOBJECT_CLASS_ID, BONE_OBJ_CLASSID)){
...
static bool HasControllerRef(map<wstring,int>& ctrlCount, const wstring& name)
{
...
static bool HasUserPropBuffer(NiNodeRef node)
{
   if (node) {
      if (NiStringExtraDataRef data = SelectFirstObjectOfType<NiStringExtraData>(node->GetExtraData())){
         if (strmatch(wide(data->GetName()), _T("UserPropBuffer")))
            return true;
      }
   }
   return false;
...
void NifImporter::ImportBones(NiNodeRef node, bool recurse)
{
   try 
   {
      ...
      NiNodeRef parent = node->GetParent();
      std::wstring parentname = wide((parent ? parent->GetName() : ""));
      Matrix44 m4 = node->GetWorldTransform();
      ...
      if (supportPrnStrings) {
         ...
         for (list<NiStringExtraDataRef>::iterator itr = strings.begin(); itr != strings.end(); ++itr){
            if (strmatch(wide((*itr)->GetName()), _T("Prn"))) {
               std::wstring parentname = wide((*itr)->GetData());
   ...
   if (node && block)
   {
      ...
      for (list<NiStringExtraDataRef>::iterator itr = strings.begin(); itr != strings.end(); ++itr){
         if (strmatch(wide((*itr)->GetName()), _T("UserPropBuffer")) || strmatch(wide((*itr)->GetName()), _T("UPB"))) {
            wchar_t buffer[1048];
            wistringstream istr(wide((*itr)->GetData()), ios_base::out);
            while (!istr.eof()) {
               wchar_t *line = buffer;
               buffer[0] = 0;
               ...
               if (LPWSTR equals = wcschr(line, '=')){
                  *equals++ = 0;
                  ...
                  if (len > 0 && line[len-1] == '#'){
                     ...
                     if (wildmatch(_T("BSBoneLOD#*"), value))
                        value.dataForWrite()[0] = 'N', value.dataForWrite()[1] = 'i'; // Use NIBoneLOD to be compatible with Civ4 code
