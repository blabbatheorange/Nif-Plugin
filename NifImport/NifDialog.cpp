      ...
      case WM_INITDIALOG:
         {
			...
			CheckDlgButton(hWnd, IDC_CHK_IMPORT_BODYTRI, imp->importBodyTri);
			...
			// Weld Threshold
			TSTR weldThresh; weldThresh.printf(_T("%g"), imp->weldVertexThresh);
      ...
      case WM_COMMAND : 
         {
            if (HIWORD(wParam) == BN_CLICKED)
            {
               ...
               case IDOK:
				  ...
				  imp->importBodyTri = IsDlgButtonChecked(hWnd, IDC_CHK_IMPORT_BODYTRI) ? true : false;
				  ...
				  if (wcslen(tmp) > 0) imp->weldVertexThresh = (float)_wtof(tmp);
               ...
               case IDC_BTN_BROWSE:
                  {
                     ...
                     pfilter = _tcscat(pfilter, _T(" (*.NIF)"));
                     pfilter += wcslen(pfilter);
                     *pfilter++ = '\0';
                     _tcscpy(pfilter, _T("*.NIF"));
                     pfilter += wcslen(pfilter);
                     *pfilter++ = '\0';
                     *pfilter++ = '\0';
