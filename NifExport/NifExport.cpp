...
int NifExport::DoExportInternal(const TCHAR *name, ExpInterface *ei, Interface *i, BOOL suppressPrompts, DWORD options)
{
   ...
   {
      nifVersion = ParseVersionString(narrow(Exporter::mNifVersion));
      if (!IsSupportedVersion(nifVersion))
      {
   ...
   {
      WriteNifTree(narrow(path), NiObjectRef(root), info);
   }
   else 
   {
      ...
      WriteFileGroup(narrow(path), StaticCast<NiObject>(root), info, export_type, game);
   }
