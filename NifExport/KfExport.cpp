   ...
   case WM_INITDIALOG:
      {
        ...
        // Append file version to dialog
         ...
         SetDlgItemText(hWnd, IDC_ED_PRIORITY, FormatText(_T("%.1f"), Exporter::mDefaultPriority));
   ...
   case WM_COMMAND:
      if (HIWORD(wParam) == BN_CLICKED)
      {
         ...
         case IDOK:
            ...
            if (tmp[0] != 0)
            {
               int nifVersion = ParseVersionString(narrow(tmp));
            ...
            Exporter::mDefaultPriority = _wtof(tmp);
...
int	KfExport::DoExport(const TCHAR *name, ExpInterface *ei, Interface *i, BOOL suppressPrompts, DWORD options)
{
   try
   {
      ...
      if (iniNameIsValid)
      {
         ...
         std::wstring curapp = GetIniValue<std::wstring>(KfExportSection, _T("CurrentApp"), _T("AUTO"), iniName);
      ...
      info.exportInfo2 = narrow((std::wstring) FormatText(_T("Niftools Max Plugins %s"), fileVersion.data()));
      WriteNifTree(narrow(name), StaticCast<NiObject>(root), info);
   }
