...
void Exporter::makeTexture(NiAVObjectRef &parent, Mtl *mtl)
{
	...
	else if (IsFallout3())
	{
		...
		if (normalStr.isNull()) {
			...
			textures[1] = narrow(mAppSettings->GetRelativeTexPath(std::wstring(normalStr), mTexPrefix));
		if (!envMaskStr.isNull())
			textures[2] = narrow(mAppSettings->GetRelativeTexPath(std::wstring(envMaskStr), mTexPrefix));
		if (!glowStr.isNull())
			textures[3] = narrow(mAppSettings->GetRelativeTexPath(std::wstring(glowStr), mTexPrefix));
		if (glowStr.isNull()){
			char path_buffer[_MAX_PATH], drive[_MAX_DRIVE], dir[_MAX_DIR], fname[_MAX_FNAME], ext[_MAX_EXT];
			_splitpath(textures[0].c_str(), drive, dir, fname, ext);
			strcat(fname, "_g");
		...
		if (!dispStr.isNull())
			textures[4] = narrow(mAppSettings->GetRelativeTexPath(std::wstring(dispStr), mTexPrefix));
		if (!envStr.isNull())
	...
	else
	{
		...
		if (mAppSettings)
		{
			std::wstring newPath = mAppSettings->GetRelativeTexPath(std::wstring(mapPath), mTexPrefix);
			imgProp->SetExternalTexture(narrow(newPath));
		}
		else
		{
			TSTR p, f;
			SplitPathFile(mapPath, &p, &f);
			TSTR newPath = (narrow(mTexPrefix) == "") ? f : (TSTR(mTexPrefix.c_str()) + _T("\\") + f);
			imgProp->SetExternalTexture(narrow(newPath.data()));
		}
		NiPropertyRef prop = DynamicCast<NiProperty>(texProp);
		parent->AddProperty(prop);
	}
...
bool Exporter::makeTextureDesc(BitmapTex *bmTex, TexDesc& td, NiTriBasedGeomDataRef shape)
{
	...
	if (UVGen *uvGen = bmTex->GetTheUVGen()){
		...
		if (RefTargetHandle ref = uvGen->GetReference(0)){
			...
			float wangle;
			bool ok = true;
			if(ok) ok &= getMAXScriptValue(ref, _T("U_Offset"), 0, trans.u);
			if(ok) ok &= getMAXScriptValue(ref, _T("V_Offset"), 0, trans.v);
			if(ok) ok &= getMAXScriptValue(ref, _T("U_Tiling"), 0, tiling.u);
			if(ok) ok &= getMAXScriptValue(ref, _T("V_Tiling"), 0, tiling.v);
			if(ok) ok &= getMAXScriptValue(ref, _T("W_Angle"), 0, wangle);
			if (ok) {
	...
	{
		std::wstring newPath = mAppSettings->GetRelativeTexPath(std::wstring(mapPath), mTexPrefix);
		td.source->SetExternalTexture(narrow(newPath));
	}
	else
	{
		TSTR p, f;
		SplitPathFile(mapPath, &p, &f);
		TSTR newPath;
		if (narrow(mTexPrefix) != "")
			newPath = TSTR(mTexPrefix.c_str()) + _T("\\") + f;
		else
			newPath = f;

		td.source->SetExternalTexture(narrow(newPath.data()));
	}
	return true;
}
      ...
      if (IsSkyrim())
      {
		...
		else if (IsFallout3())
		{
			...
			else if (BSShaderNoLightingPropertyRef texProp = StaticCast<BSShaderNoLightingProperty>(root))
			{
				...
				texProp->SetFileName( narrow(diffuseStr.data()) );
				NiPropertyRef prop = DynamicCast<NiProperty>(texProp);
				parent->AddProperty(prop);
				useDefaultShader = false;
			}
