...
static LPCWSTR NifExportSection = TEXT("MaxNifExport");
static LPCWSTR KfExportSection = TEXT("KfExport");
static LPCWSTR CollisionSection = TEXT("Collision");
...
void regGet(HKEY hKey, const wchar_t *value, int &v)
{
	DWORD dw, type, cdata = sizeof(DWORD);
    if (RegQueryValueEx(hKey, value, NULL, &type, (LPBYTE)&dw, &cdata)==ERROR_SUCCESS && type==REG_DWORD)
		v = dw;
