...
TSTR FormatText(const TCHAR* format,...)
{
   ...
   if (nChars != -1) {
      ...
      text.Resize(Size);
      nChars = _vsntprintf((wchar_t*)text.data(), Size, format, args);
   }
   va_end(args);
   return text;
}
...
stringlist TokenizeCommandLine(LPCWSTR str, bool trim)
{
   ...
   for (int i=0; i<nargs; ++i) {
      ...
      values.push_back(std::wstring((trim) ? Trim(val) : val));
   }
   return values;
}
...
inline TSTR TOSTRING(const Point3& p) {
   return FormatText(_T("[%g,%g,%g]"), p[0], p[1], p[2]);
}

inline TSTR TOSTRING(float p) {
   return FormatText(_T("%g"), p);
}
...
vector<std::wstring> GetNamesOfNodes( const vector<Niflib::NiNodeRef>& nodes )
{
   vector<std::wstring> slist;
...
TSTR PrintMatrix3(Matrix3& m)
{
   ...
   return FormatText(_T("Matrix3: [%g,%g,%g] <%g,%g,%g>\n")
      ,pt.x, pt.y, pt.z
...
TSTR PrintMatrix44(Matrix44& m)
{
   ...
   return FormatText(_T("Matrix3: [%g,%g,%g] <%g,%g,%g> (%g)\n")
      ,p.x, p.y, p.z
...
{
   return i->GetINodeByName(wide(name).c_str());
}
...
TSTR GetFileVersion(const wchar_t *fileName)
{
   ...
   if (!ver) ver = LoadLibrary(_T("version.dll"));
   if (ver != NULL)
   {
...
void GetIniFileName(wchar_t *iniName)
{
	...
	if (_waccess(iniName, 06) != 0) {
		MessageBox(NULL, _T("MaxNifTools could not find a valid INI.  The plugin may not work correctly.\nPlease check for proper installation."), 
			_T("MaxNifTools"), MB_OK|MB_ICONWARNING);
	}
}
...
int StringToEnum(TSTR value, const EnumLookupType *table) {
   ...
   for (const EnumLookupType *itr = table; itr->name != NULL; ++itr) {
      if (0 == _wcsicmp(value, itr->name)) return itr->value;
   }
   wchar_t *end = NULL;
