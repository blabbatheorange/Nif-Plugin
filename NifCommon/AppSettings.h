...
public:
   ...
   template<>
   inline std::wstring GetSetting(std::wstring setting, std::wstring Default){
      NameValueCollection::iterator itr = Environment.find(setting);
      if (itr != Environment.end())
         return (*itr).second;
      return Default;
...
struct AppSettingsNameEquivalence : public ltstr
{
   ...
   bool operator()(const std::wstring& n1, const AppSettings& n2) const { 
      return ltstr::operator()(n1, n2.Name);
   }
   bool operator()(const AppSettings& n1, const std::wstring& n2) const { 
